...
const Buttons = ({ wallet }: { wallet: ReturnType<typeof useWallet> }) => {
  const next = () => {
    var x = parseInt(document.getElementById('ship1x').value);
    var y = parseInt(document.getElementById('ship1y').value);
    wallet?.contract.turn(x,y,main.myShip());
  }
  const next2 = () => {
    var x = parseInt(document.getElementById('ship2x').value);
    var y = parseInt(document.getElementById('ship2y').value);
    wallet?.contract.turn(x,y,main.myShip2());
  }
  const register = () => {
    wallet?.contract.register(main.myShip())
  }
  const register2 = () => {
    wallet?.contract.register(main.myShip2())
  }
  return (
    <div style={{ display: 'flex', gap: 5, padding: 5 }}>
      <div style={{ flex:1, display:'flex', gap:4, flexDirection:'column' }}>
        <button style={{ flex:1 }} onClick={register}>Register player 1</button>
        <label style={{ flex:2 }}>x: <input id="ship1x" type="text" /></label>
        <label style={{ flex:3 }}>y: <input id="ship1y" type="text" /></label>
        <button style={{ flex:4 }} onClick={next}>Turn</button>
      </div>
      <div style={{ flex:2, display:'flex', gap:4, flexDirection:'column' }}>
        <button style={{ flex:1 }} onClick={register2}>Register player 2</button>
        <label style={{ flex:2 }}>x: <input id="ship2x" type="text"/></label>
        <label style={{ flex:3 }}>y: <input id="ship2y" type="text"/></label>
        <button style={{ flex:4 }} onClick={next2}>Turn</button>
      </div>
    </div>
  )
}
    ...
    <div className={styles.body}>
      ...
      <div className={styles.grid} style={st}>
        {CELLS.fill(0).map((_, index) => {
          ...
          return (
            <div onClick={ (e) => update(x, y) } key={index} className={styles.cell} style={{ background }} />
          )
        })}
      </div>
